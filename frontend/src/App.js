...
function ProductCard({ p, onAdd }) {
  ...
  return <article className={`product ${disabled ? "product-off" : ""}`} data-testid={`product-card-${slug(p.name)}`}>
    <div className="product-photo">
      <img src={p.image_url} alt={`Imagem ilustrativa de ${p.name}`} />
      <span>{p.category}</span>
      {disabled && <div className="soldout-badge" data-testid={`soldout-${slug(p.name)}`}>Esgotado</div>}
    </div>
    <div className="product-body">
      ...
      <div className="product-bottom">
        ...
        <button
          className="add"
          data-testid={`add-${slug(p.name)}`}
          disabled={disabled}
          onClick={() => !disabled && onAdd(p)}
        ><Plus size={17} />{disabled ? "Indisponível" : "Adicionar"}</button>
      </div>
    </div>
  </article>;
}
...
function App() {
  ...
  const sendWhatsApp = async () => {
    ...
    } catch (e) { /* backend log is a nice-to-have; still forward to WhatsApp */ }
  ...
  return <div className="site">
    ...
    {step === "catalog" && <main>
      <section className="hero">
        ...
        <div className="hero-image">
          <img src={products[0]?.image_url || logo} alt="Bolos fresquinhos da padaria" />
      ...
      <section className="catalog" id="catalogo">
        ...
        {loading ? <p data-testid="loading-products" style={{ color: "var(--muted)", padding: "40px 0" }}>Carregando cardápio...</p> :
          <div className="products">{visible.map(p => <ProductCard key={p.id} p={p} onAdd={add} />)}</div>}
      </section>
    </main>}
    ...
    {step === "cart" && <section className="flow">
      ...
      {cart.length === 0 ? <div className="empty" data-testid="empty-cart-message"><ShoppingBag size={42} /><h2>Seu pedido está vazio</h2><button className="hero-action" onClick={() => setStep("catalog")} data-testid="empty-cart-browse-button">Ver cardápio</button></div> :
        <div className="cart-layout"><div className="cart-list">
          ...
          <button className="continue" data-testid="continue-checkout-button" onClick={() => setStep("checkout")}>Continuar <ArrowRight size={18} /></button>
        </div></div>}
    </section>}
    ...
    {step === "checkout" && <section className="flow">
      ...
      <form className="checkout" onSubmit={e => { e.preventDefault(); sendWhatsApp(); }}>
        ...
        <div className="checkout-footer"><div><span>Total dos produtos</span><strong>{money(total)}</strong></div><button className="whatsapp" data-testid="send-whatsapp-button" type="submit"><MessageCircle size={20} />Enviar pedido pelo WhatsApp</button></div>
      </form>
    </section>}
